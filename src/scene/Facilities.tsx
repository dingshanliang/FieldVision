...
function PumpStation() {
  ...
  return (
    <group position={[82, 1.6, 108]} rotation={[0, -0.16, 0]}>
      ...
      {[-3.2, -0.6].map((x) => (
        <mesh key={x} position={[x, 4.6, 4.42]}>
...
function Gate() {
  ...
  return (
    <group position={[69, 1.2, -18]} rotation={[0, 0.18, 0]}>
      {[-2.9, 2.9].map((x) => (
        <mesh key={x} position={[x, 2.3, 0]} castShadow receiveShadow>
          <boxGeometry args={[1.1, 4.8, 1.7]} />
          <meshStandardMaterial color="#8b8d84" roughness={0.85} envMapIntensity={0.35} />
        </mesh>
      ))}
      {/* guide rails */}
      {[-2.32, 2.32].map((x) => (
      ...
      <mesh position={[0, 5.05, 0]} castShadow>
        ...
        <meshStandardMaterial color="#3f4744" metalness={0.35} roughness={0.5} envMapIntensity={0.5} />
      </mesh>
      {/* rising gate leaf */}
      ...
      <mesh position={[0, 5.85, 0.75]} rotation={[Math.PI / 2, 0, 0]}>
