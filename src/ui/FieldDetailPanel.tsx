...
export function FieldDetailPanel() {
  ...
  const recoveredMoisture = Math.round(field.moisture + (field.moistureTarget - field.moisture) * progress);
  ...
  return (
    <aside className="field-panel" aria-live="polite">
      ...
      <div className="metric-pair">
        ...
        <div><small>长势指数</small><strong>{(field.growthIndex + progress * 0.07).toFixed(2)}</strong><span>{progress > 0.8 ? "持续回升" : "近 7 日"}</span></div>
      </div>
      {field.id === "A02" && status !== "recovered" && (
        <div className={`risk-note ${status === "processing" ? "risk-note--processing" : ""}`}><ScanLine size={17} /><p><strong>{status === "processing" ? "灌溉恢复中" : "局部缺水风险"}</strong><span>{status === "processing" ? "东支渠已进水，墒情与长势正在同步恢复。" : "长势低于同品种平均值 12%，东支渠供水异常。"}</span></p></div>
      )}
